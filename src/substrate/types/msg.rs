...
impl WasmHeader for SignedBlockWithAuthoritySet {
    ...
    fn to_wasm_create_msg(&self, cfg: &CosmosConfig, address: String, client_id:String) -> Result<Vec<Value>, Box<dyn Error>> {
        ...
        Ok(vec![serde_json::json!({"type": MsgCreateWasmClient::<SignedBlockWithAuthoritySet>::get_type(), "value": &msg})])
    }
    ...
    fn to_wasm_update_msg(&self, address: String, client_id: String) -> Vec<Value> {
        ...
        vec![serde_json::json!({"type": MsgUpdateWasmClient::<SignedBlockWithAuthoritySet>::get_type(), "value": &msg})]
    }
}
